...
row: fields {
        ...
        char *json_row = (char *) malloc(strlen($1) + 21);

        sprintf(json_row, "  <item>\n%s\n  </item>", $1);
...
field: STRING {
        ...
        if (header_count > 0) {
            ...
            char *json_field = (char *) malloc(2 * strlen(headers[original_header_count - header_count]) + strlen($1) + 12);


            sprintf(json_field, "    <%s> %s <%s>", headers[original_header_count - header_count], $1, headers[original_header_count - header_count]);
...
int main() {
    ...
    output_file = fopen("output.xml", "w");
